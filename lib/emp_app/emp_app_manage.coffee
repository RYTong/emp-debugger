...
class emp_app
  ...
  constructor: (tmp_emp_app_view)->
    ...
    unless atom.config.get(EMP_MAKE_CMD_KEY)
      atom.config.set(EMP_MAKE_CMD_KEY, emp_app_make_cmd)
    # else
    #   emp_app_make_cmd = tmp_emp_make

    unless atom.config.get(EMP_STAET_SCRIPT_KEY)
      atom.config.set(EMP_STAET_SCRIPT_KEY, emp_app_start_script)
    # else
    #   emp_app_start_script = tmp_emp_start_sc

    unless atom.config.get(EMP_CONFIG_KEY)
      atom.config.set(EMP_CONFIG_KEY, emp_app_config_cmd)
    # else
    #   emp_app_config_cmd = tmp_emp_config_sc
    # app_state = false

    unless atom.config.get(EMP_CONFIG_ARG_KEY)
      atom.config.set(EMP_CONFIG_ARG_KEY, emp_app_config_arg)
  ...
  make_app: ->
    # console.log "make"
    make_str = atom.config.get(EMP_MAKE_CMD_KEY)
  ...
  config_app: ->
    # console.log "config"
    conf_file = atom.config.get(EMP_CONFIG_KEY)
    conf_ags = atom.config.get(EMP_CONFIG_ARG_KEY)
  ...
  run_app: ->
    # console.log "run"
    script_file = atom.config.get(EMP_STAET_SCRIPT_KEY)
