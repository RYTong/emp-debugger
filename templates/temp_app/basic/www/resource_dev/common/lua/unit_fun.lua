globalTable = {};
--<atom_emp_related_file_info>public/www/resource_dev/common/lua/unit_fun.lua</atom_emp_related_file_info>--
...
function invoke_trancode(channelId, tranCode, postParams, busiCallback, callbackParams)
    ...
    if ewp_debug then
        ...
        if callbackParams.login_auth == "true" then
            ryt:post(nil, "ebank_s/login_filter", client_post, all_callback, params, false);
        else
...
--[[
    ...
    说明：请求接口所需参数列表
    格式：table
    例子：{id="ebank_bkrytong",type=collections}
    busiCallback:
    说明：post请求的回调方法。
    格式：function 名称。
    例子：funCallback
...
function invoke_page(ebank_file,fun_callback,fun_params)
    ...
    if page_ewp_debug then
        ...
        --local path = "name="..utility:escapeURI("channels/"..ebank_file).."&platform=qt&resolution=1*1";
...
function jc_test(msg)
    local ll = "this is test~!";
    print(ll);
